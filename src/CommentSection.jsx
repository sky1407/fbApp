    ...
    <div className="mt-6 space-y-4">
        ...
        {displayedComments.map(comment => {
          ...
          return (
            <div key={comment.id} className="bg-gray-50 dark:bg-zinc-800/50 border border-gray-100 dark:border-zinc-800 rounded-xl p-3.5 space-y-2">
              ...
              {/* Editačný režim / Zobrazenie komentára */}
              {isEditingThisComment ? (
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <button type="button" onClick={() => handleSaveEditComment(post.id, comment.id)} className="px-2.5 py-1 bg-green-600 hover:bg-green-700 text-white text-xs font-medium rounded transition cursor-pointer">Uložiť</button>
                    <button type="button" onClick={() => startEditComment(null)} className="px-2.5 py-1 bg-gray-500 hover:bg-gray-600 text-white text-xs font-medium rounded transition cursor-pointer">Zrušiť</button>
                  </div>
                </div>
              ) : (
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs font-semibold text-gray-500 dark:text-gray-400">
                ...
                {!isEditingThisComment && <button type="button" onClick={() => startEditComment(comment)} className="hover:text-gray-700 dark:hover:text-zinc-300 cursor-pointer">Upraviť</button>}
                <button type="button" onClick={() => handleDeleteComment(post.id, comment.id)} className="text-red-500 hover:underline cursor-pointer">Vymazať</button>
                {comment.replies && comment.replies.length > 0 && (
                  <button type="button" onClick={() => toggleCommentReplies(comment.id)} className="text-gray-700 dark:text-zinc-300 font-bold hover:underline cursor-pointer">
                    {isCommentExpanded ? `Skryť odpovede` : `Zobraziť odpovede (${comment.replies.length})`}
                  </button>
                )}
              </div>

              {/* Vykreslenie reakcií (2. úroveň) */}
              {isCommentExpanded && comment.replies && comment.replies.map(reply => (
                <div key={reply.id} className="bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-lg p-3 ml-6 mt-2 space-y-1.5 shadow-xs">
                  ...
                  <div className="flex gap-3 text-[11px] font-semibold text-gray-500 dark:text-gray-400">
                    ...
                    <button type="button" onClick={() => handleDeleteReply(post.id, comment.id, reply.id)} className="text-red-500 hover:underline cursor-pointer">Vymazať</button>
                  </div>
                </div>
              ))}

              {/* Formulár pre pridanie reakcie */}
              <div className="ml-6 mt-3 flex gap-2 items-center">
                ...
                />
                <button type="button" onClick={() => handleAddReply(post.id, comment.id)} className="px-3 py-1.5 bg-gray-200 dark:bg-zinc-700 hover:bg-gray-300 dark:hover:bg-zinc-600 text-gray-700 dark:text-zinc-200 text-xs font-semibold rounded-lg transition-colors cursor-pointer shrink-0">Odpovedať</button>
              </div>
      ...
      {hasMoreComments && (
        <button type="button" onClick={() => togglePostComments(post.id)} className="w-full text-center py-2 bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 text-gray-700 dark:text-zinc-300 text-xs font-bold rounded-lg transition-colors cursor-pointer">
          {isExpanded ? 'Zbaliť komentáre' : `Zobraziť ďalšie komentáre (${post.comments.length - 2})`}
        </button>
      )}

      {/* Formulár pre pridanie hlavného komentára */}
      <div className="flex gap-2 items-start pt-2">
        ...
        />
        <button type="button" onClick={() => handleAddComment(post.id)} className="px-4 py-2.5 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold rounded-lg shadow-xs transition-colors cursor-pointer shrink-0">Komentovať</button>
      </div>
    </div>
  );
}
